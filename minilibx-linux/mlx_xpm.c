...
char	*mlx_int_get_line(char *ptr,int *p_pos,int size)
{
	int			pos2;
	int			pos3;
	int			pos4;

	if ((pos2 = mlx_int_str_str(ptr+*p_pos,"\"",size-*p_pos))==-1)
		return ((char *)0);
	if ((pos3 = mlx_int_str_str(ptr+*p_pos+pos2+1,"\"",size-*p_pos-pos2-1))==-1)
		return ((char *)0);
	*(ptr+*p_pos+pos2) = 0;
	*(ptr+*p_pos+pos2+1+pos3) = 0;
	pos4 = *p_pos+pos2+1;
	*p_pos += pos2+pos3+2;
	return (ptr+pos4);
}
...
char	*mlx_int_static_line(char **xpm_data,int *p_pos,int size)
{
	static char	*copy = 0;
	static int	len = 0;
	int			len2;
...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		tab = 0;
		p_pos = 0;
		if (!(line = f(info,&p_pos,info_size)) ||
